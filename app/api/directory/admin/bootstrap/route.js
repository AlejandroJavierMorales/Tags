...
export async function GET() {
    ...
    const [sites,taxonomy,places,businesses,listings] = await Promise.all([
        ...
        db.query("SELECT n.*,p.name parent_name,(SELECT COUNT(*) FROM tags_directory_listing_taxonomy lt WHERE lt.taxonomy_node_id=n.id) listing_count,NOT EXISTS (SELECT 1 FROM tags_directory_taxonomy_nodes child WHERE child.parent_id=n.id) is_leaf FROM tags_directory_taxonomy_nodes n LEFT JOIN tags_directory_taxonomy_nodes p ON p.id=n.parent_id ORDER BY n.name,n.sort_order"),
        db.query("SELECT g.*,p.name parent_name,(SELECT COUNT(*) FROM tags_directory_listing_places lp WHERE lp.place_id=g.id) listing_count FROM tags_geo_places g LEFT JOIN tags_geo_places p ON p.id=g.parent_id ORDER BY g.name"),
