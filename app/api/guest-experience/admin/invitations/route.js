...
export async function POST(req) {
    ...
    const [rows] = await db.query(`SELECT s.id,s.primary_guest_id,s.stay_code,s.starts_at,s.ends_at,s.adults,s.children,s.expected_arrival_text,s.lodging_total,s.deposit_required_amount,a.slug,a.name app_name,a.logo_url,a.currency,a.settings_json,u.name unit_name,g.name guest_name,g.email,g.phone FROM tags_guest_stays s INNER JOIN tags_guest_apps a ON a.id=s.guest_app_id AND a.business_id=? INNER JOIN tags_guest_people g ON g.id=s.primary_guest_id LEFT JOIN tags_guest_units u ON u.id=s.unit_id WHERE s.id=? AND s.guest_app_id=? LIMIT 1`, [businessId, stayId, guestAppId]);
    const stay = rows[0];
    ...
    const finalCommunication = buildReservationAccessCommunication({ stay, settings, link, eventCode });
    let status = "prepared", sentAt = null, lastError = null, providerReference = null, emailSent = false;
    if (channel === "email") {
        const delivery = await sendMail({ to: stay.email, subject: finalCommunication.title, html: finalCommunication.html, text: finalCommunication.text });
        ...
        if (delivery.ok) {
            ...
            const guestEmail = String(stay.email || "").trim().toLowerCase();
            const recipients = [...new Set(admins.map(item => String(item.email || "").trim().toLowerCase()).filter(email => email && email !== guestEmail))];
            if (recipients.length) {
                const copy = await sendMail({ to: recipients, subject: `Copia · ${finalCommunication.title}`, html: finalCommunication.html, text: finalCommunication.text });
                if (!copy.ok) console.error("GUEST RESERVATION PLATFORM COPY ERROR", copy.error);
            }
        }
    } else if (channel === "manual") status = "generated";
    const phone = normalizeArgentinaWhatsapp(stay.phone);
    const whatsappUrl = phone ? `https://wa.me/${phone}?text=${encodeURIComponent(finalCommunication.text)}` : null;
    await db.query("INSERT INTO tags_guest_communications (guest_app_id,stay_id,guest_id,access_token_id,event_code,direction,channel,recipient,subject,message_text,status,sent_at,failed_at,attempts,provider_reference,last_error,created_by_type,created_by_id) VALUES (?,?,?,? ,?,'outbound',?,?,?,?,?,?,?,1,?,?, 'owner',?)", [guestAppId, stayId, stay.primary_guest_id, tokenId, eventCode, channel, channel === "email" ? stay.email : stay.phone, finalCommunication.title, finalCommunication.text, status, sentAt, status === "failed" ? new Date() : null, providerReference, lastError, access.session?.id || access.session?.userId || null]);
