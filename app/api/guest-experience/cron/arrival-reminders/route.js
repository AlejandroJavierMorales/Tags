...
async function runArrivalReminders(req) {
    ...
    for (const stay of rows) {
        ...
        const eventCode = remaining <= 24 * 3600000 ? "arrival_reminder_24h" : "arrival_reminder";
        const [alreadySent] = await db.query("SELECT id FROM tags_guest_communications WHERE stay_id=? AND event_code=? AND status='sent' LIMIT 1", [stay.id, eventCode]);
        ...
        const subject = eventCode === "arrival_reminder_24h" ? `Tu ingreso es mañana · ${stay.app_name}` : "¡Tenés un ingreso a nuestras cabañas!";
