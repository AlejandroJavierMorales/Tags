...
export async function GET(req) {
    ...
    const[rows]=await db.query("SELECT id,sector,network_name,password,instructions FROM tags_guest_wifi_networks WHERE guest_app_id=? AND is_active=1 ORDER BY sort_order,id",[session.id]);
    const settings=parseGuestJson(session.settings_json);
    return Response.json({ok:true,wifiNetworks:rows.map(item=>({id:item.id,sector:item.sector,networkName:item.network_name,password:item.password,instructions:item.instructions})),information:{checkinTime:settings.checkinTime||"",checkoutTime:settings.checkoutTime||"",receptionPhone:settings.receptionPhone||"",receptionEmail:settings.receptionEmail||"",arrivalInstructions:settings.arrivalInstructions||"",departureInstructions:settings.departureInstructions||"",houseRules:settings.houseRules||""}});
}
