...
export async function POST(req) {
    ...
    const [rows] = await db.query(`SELECT c.id,c.name,c.email,a.id agency_id,a.slug,a.magic_link_minutes,b.display_name,b.name business_name,b.logo_url FROM tags_qr_agency_customers c INNER JOIN tags_qr_agencies a ON a.id=c.agency_id AND a.business_id=? AND a.status='active' INNER JOIN tags_businesses b ON b.id=a.business_id WHERE c.id=? AND c.status='active' LIMIT 1`, [businessId, customerId]);
    const item = rows[0];
    ...
    const delivery = await sendMail({
        to: item.email,
        subject: `Acceso a tus códigos QR · ${agencyName}`,
        html: `<div style="max-width:600px;margin:auto;font-family:Arial;color:#183226"><div style="padding:22px;background:#f2f8f4;text-align:center"><img src="${html(logoUrl)}" alt="Tags" style="max-width:170px;max-height:85px"><h1 style="font-size:24px">Administrá tus códigos QR</h1></div><div style="padding:24px"><p>Hola <strong>${html(item.name)}</strong>,</p><p>${html(agencyName)} te envía un acceso privado para administrar tus códigos QR y consultar tus estadísticas.</p><p style="text-align:center;margin:28px"><a href="${html(link)}" style="display:inline-block;padding:13px 20px;background:#198754;color:#fff;text-decoration:none;border-radius:9px;font-weight:bold">Ingresar a mi panel</a></p><p style="font-size:12px;color:#66766e">Este enlace vence en ${minutes} minutos y puede utilizarse una sola vez.</p><p style="font-size:12px;color:#66766e">Para volver a solicitar acceso, guardá esta dirección: <a href="${html(loginUrl)}">${html(loginUrl)}</a></p></div></div>`,
