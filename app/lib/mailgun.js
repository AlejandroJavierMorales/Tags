...
export async function sendMagicLink(email, link, branding = {}) {
  ...
  const isPersonalAccount = branding.accountType === "user";
  const subject = isPersonalAccount ? `Acceso a tu cuenta de ${name}` : `Acceso a tu panel de ${name}`;
  const plainTarget = isPersonalAccount ? "tu cuenta personal" : "tu panel";
  const htmlTarget = isPersonalAccount ? "tu cuenta personal" : "tu Panel de Administración";
  const buttonLabel = isPersonalAccount ? "Ingresar a mi cuenta" : "Ingresar a mi panel";

  return mg.messages.create(sendingDomain, {
    from,
    to: [email],
    subject,
    text: `Ingresá a ${plainTarget} desde este enlace: ${link}\n\nEl enlace expira en 15 minutos y puede utilizarse una sola vez.`,
    html: `
      <div style="margin:0;padding:24px;background:#f3f7f4;font-family:Arial,sans-serif;color:#24332a">
        <div style="max-width:560px;margin:auto;background:#fff;border:1px solid #dfe9e2;border-radius:12px;overflow:hidden">
          ...
          <div style="padding:30px;text-align:center">
            ...
            <p style="margin:0 0 24px;color:#5c6d62">Hacé clic en el botón para ingresar a ${htmlTarget}.</p>
            <p style="margin:0 0 24px"><a href="${safeLink}" target="_blank" style="display:inline-block;padding:13px 24px;background:${escapeHtml(color)};color:#fff;text-decoration:none;border-radius:8px;font-weight:700">${buttonLabel}</a></p>
