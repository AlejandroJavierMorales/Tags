...
export default function AiChatSurfaceSettings({ businessId, surfaceType, surfaceId, surfaceLabel = "esta página" }) {
    ...
    return <form className="tags_ai_surface_panel" onSubmit={save}>
        ...
        <div className="tags_ai_surface_grid">
            <label>Imagen de acceso<select value={form.widget_type} onChange={event => update("widget_type", event.target.value)}><option value="bubble">Burbuja de chat</option><option value="robot">Robot</option><option value="avatar_woman">Asistente mujer</option><option value="avatar_man">Asistente hombre</option></select><small>Elegí cómo se verá el botón para abrir el chatbot.</small></label>
