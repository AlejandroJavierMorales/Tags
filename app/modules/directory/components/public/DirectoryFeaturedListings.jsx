...
export default function DirectoryFeaturedListings({ listings }) {
  ...
  return <section className="tags_directory_featured" aria-labelledby="directory-featured-title">
    ...
    <div className="tags_directory_featured_grid">{listings.map(listing => <article key={listing.id}>
      ...
      <div className="tags_directory_featured_body"><h3>{listing.display_name}</h3>{listing.locality_name && <p><FaLocationDot /> {listing.locality_name}</p>}<small>{listing.short_description}</small><Link href={`/directorio/prestador/${listing.slug}`}>Conocer más <FaArrowRight /></Link></div>
    </article>)}</div>
  </section>;
}
