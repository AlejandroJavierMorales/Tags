...
import { getPublicLoyaltyProgramByBusiness } from "@/app/modules/loyalty/lib/loyaltyPublicService";
...
export async function getDirectoryWebPageData(pageId, { includeDraft = false } = {}) {
    ...
    const page = normalizeDirectoryDbValue(pages[0]);
    if (!page) return null;
    ...
    const [sectionResult, blockResult, productResult, embeddedStore, embeddedResto, embeddedReviews, benefitResult, loyalty] = await Promise.all([
        ...
        getPublicLoyaltyProgramByBusiness(page.business_id).catch(error => {
            if (["ER_NO_SUCH_TABLE", "ER_BAD_FIELD_ERROR"].includes(error?.code)) return null;
            throw error;
        })
    ]);
    const sections = normalizeDirectoryDbValue(sectionResult[0]);
    const blocks = normalizeDirectoryDbValue(blockResult[0]);
    const products = normalizeDirectoryDbValue(productResult[0]);
    const benefits = normalizeDirectoryDbValue(benefitResult[0]);

    return normalizeDirectoryDbValue({
        ...
        sections: sections.map(section => ({ ...section, settings_json: safeParseJSON(section.settings_json), styles_json: safeParseJSON(section.styles_json), blocks: blocks.filter(block => Number(block.section_id) === Number(section.id)).map(block => ({ ...block, content_json: safeParseJSON(block.content_json), styles_json: safeParseJSON(block.styles_json) })) })),
