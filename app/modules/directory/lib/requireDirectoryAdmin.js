...
export async function requireDirectoryAdmin() {
    ...
    try {
        ...
        return session.role === "admin" ? { ok: true, session } : { ok: false, status: 403, error: "Sin permisos" };
    } catch { return { ok: false, status: 401, error: "Sesión inválida" }; }
}

export function directoryAdminError(access) {
